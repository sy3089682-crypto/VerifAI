...
export function detectDrift(prompt, output) {
  ...
  if (promptLower.includes('async') && !outputLower.includes('async') && !outputLower.includes('await')) {
    drifts.push({
  ...
  if (promptLower.includes('error') && !outputLower.includes('error') && !outputLower.includes('try')) {
    drifts.push({
  ...
  if (promptLower.includes('test') && !outputLower.includes('test') && !outputLower.includes('describe') && !outputLower.includes('it(')) {
    drifts.push({
      type: 'missing-tests',
  ...
  if (promptLower.includes('secure') || promptLower.includes('security')) {
    if (outputLower.includes('innerhtml') || outputLower.includes('eval(')) {
      drifts.push({
        ...
        message: 'Security was requested but output contains unsafe patterns (innerHTML/eval)',
        severity: 'high',
      });
    }
  }
